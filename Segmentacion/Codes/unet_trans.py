...
# Cargar las imágenes transversales
X_imagenes_trans, Y_etiquetas_trans = cargar_imagenes('/home/tfd/mgutierr/tfm/DATA-Tunnel-Trans/file_info.json', etiquetas_unicas, 'transversal')
...
X_roi_trans, Y_roi_trans = cargar_mascaras('/home/tfd/mgutierr/tfm/DATA-Tunnel-Trans/file_info.json', etiquetas_unicas, 'transversal')

X_imagenes = X_imagenes_trans 
Y_etiquetas = Y_etiquetas_trans
X_roi = X_roi_trans
Y_roi = Y_roi_trans 
...
X_masks_one_hot = to_categorical(X_mask, num_classes=5) # Estas son las etiquetas, que en vd son máscaras en formato one-hot
...
kernel_regularizer_encoder = tf.keras.regularizers.l2(0.0001)
kernel_regularizer_decoder = tf.keras.regularizers.l2(0.0001)
...
c1 = tf.keras.layers.Conv2D(16, (3,3), activation='relu', kernel_initializer='he_normal', padding='same', kernel_regularizer=kernel_regularizer_encoder)(s) 
    # he_normal is to start the starting weights, it defines the initial value. It has to do with normal 
    # distribution, that is gaussian. There are more like truncated nomal..., it depends on what u want to do
c1 = tf.keras.layers.Dropout(0.4)(c1)
c1 = tf.keras.layers.Conv2D(16, (3,3), activation='relu', kernel_initializer='he_normal', padding='same', kernel_regularizer=kernel_regularizer_encoder)(c1) 
p1 = tf.keras.layers.MaxPooling2D((2,2))(c1)

c2 = tf.keras.layers.Conv2D(32, (3,3), activation='relu',kernel_initializer='he_normal', padding='same', kernel_regularizer=kernel_regularizer_encoder) (p1)
c2 = tf.keras.layers.Dropout(0.4)(c2)
c2 = tf.keras.layers.Conv2D(32, (3,3), activation='relu', kernel_initializer='he_normal', padding='same', kernel_regularizer=kernel_regularizer_encoder)(c2)
p2 = tf.keras.layers.MaxPool2D((2,2))(c2)

c3 = tf.keras.layers.Conv2D(64, (3,3), activation='relu',kernel_initializer='he_normal', padding='same', kernel_regularizer=kernel_regularizer_encoder) (p2)
c3 = tf.keras.layers.Dropout(0.4)(c3)
c3 = tf.keras.layers.Conv2D(64, (3,3), activation='relu', kernel_initializer='he_normal', padding='same', kernel_regularizer=kernel_regularizer_encoder)(c3)
p3 = tf.keras.layers.MaxPool2D((2,2))(c3)

c4 = tf.keras.layers.Conv2D(128, (3,3), activation='relu',kernel_initializer='he_normal', padding='same', kernel_regularizer=kernel_regularizer_encoder) (p3)
c4 = tf.keras.layers.Dropout(0.4)(c4)
c4 = tf.keras.layers.Conv2D(128, (3,3), activation='relu', kernel_initializer='he_normal', padding='same', kernel_regularizer=kernel_regularizer_encoder)(c4)
p4 = tf.keras.layers.MaxPool2D((2,2))(c4)

c5 = tf.keras.layers.Conv2D(256, (3,3), activation='relu',kernel_initializer='he_normal', padding='same', kernel_regularizer=kernel_regularizer_encoder) (p4)
c5 = tf.keras.layers.Dropout(0.4)(c5)
c5 = tf.keras.layers.Conv2D(256, (3,3), activation='relu', kernel_initializer='he_normal', padding='same', kernel_regularizer=kernel_regularizer_encoder)(c5)

# Expansive path (decoder)

u6 = tf.keras.layers.Conv2DTranspose(128, (2,2), strides=(2,2), padding='same')(c5)
u6 = tf.keras.layers.concatenate([u6,c4])
c6 = tf.keras.layers.Conv2D(128, (3,3), activation='relu', kernel_initializer='he_normal', padding='same', kernel_regularizer=kernel_regularizer_encoder)(u6)
c6 = tf.keras.layers.Dropout(0.3)(c6)
c6 = tf.keras.layers.Conv2D(128, (3,3), activation='relu', kernel_initializer='he_normal', padding='same', kernel_regularizer=kernel_regularizer_encoder)(c6)

u7 = tf.keras.layers.Conv2DTranspose(64, (2,2), strides=(2,2), padding='same')(c6)
u7 = tf.keras.layers.concatenate([u7,c3])
c7 = tf.keras.layers.Conv2D(64, (3,3), activation='relu', kernel_initializer='he_normal', padding='same', kernel_regularizer=kernel_regularizer_encoder)(u7)
c7 = tf.keras.layers.Dropout(0.3)(c7)
c7 = tf.keras.layers.Conv2D(64, (3,3), activation='relu', kernel_initializer='he_normal', padding='same', kernel_regularizer=kernel_regularizer_encoder)(c7)

u8 = tf.keras.layers.Conv2DTranspose(32, (2,2), strides=(2,2), padding='same')(c7)
u8 = tf.keras.layers.concatenate([u8,c2])
c8 = tf.keras.layers.Conv2D(32, (3,3), activation='relu', kernel_initializer='he_normal', padding='same', kernel_regularizer=kernel_regularizer_encoder)(u8)
c8 = tf.keras.layers.Dropout(0.3)(c8)
c8 = tf.keras.layers.Conv2D(32, (3,3), activation='relu', kernel_initializer='he_normal', padding='same', kernel_regularizer=kernel_regularizer_encoder)(c8)

u9 = tf.keras.layers.Conv2DTranspose(16, (2,2), strides=(2,2), padding='same')(c8)
u9 = tf.keras.layers.concatenate([u9,c1])
c9 = tf.keras.layers.Conv2D(16, (3,3), activation='relu', kernel_initializer='he_normal', padding='same', kernel_regularizer=kernel_regularizer_encoder)(u9)
c9 = tf.keras.layers.Dropout(0.3)(c9)
c9 = tf.keras.layers.Conv2D(16, (3,3), activation='relu', kernel_initializer='he_normal', padding='same', kernel_regularizer=kernel_regularizer_encoder)(c9)

outputs = tf.keras.layers.Conv2D(5, (1,1), activation='softmax')(c9)

callbacks = [
    tf.keras.callbacks.EarlyStopping(patience=3, monitor='val_loss')] # for nowing the amount of epochs which are efficient, if there is no improvement then it stops
...
model.save(f'/home/tfd/mgutierr/tfm/U-net/Codigos/unet_trans/modelo_unet_trans_{n}.h5')

# Guardar la arquitectura
estructura_json = model.to_json()
with open(f"/home/tfd/mgutierr/tfm/U-net/Codigos/unet_trans/estructura_modelo_{n}.json", "w") as json_file:
...
IOU_keras = tf.keras.metrics.MeanIoU(num_classes=5)
...
values = np.array(IOU_keras.get_weights()).reshape(5,5)
print(values)
...
class4_IoU = values[4,4]/(values[4,4] + values[4,0] + values[4,1] + values[4,2] + values[4,3])
...
f1_scores = []
for i in range(5):  # Asumiendo 5 clases
...
class_legend = [
    ...
    f'IoU for class 4: {class4_IoU:.4f}',
    ...
    f'F1-Score for class 4: {f1_scores[4]:.4f}'
]
...
sns.heatmap(
    ...
    fmt="g", 
    cmap="Blues", 
    xticklabels=['Fondo', 'Nervio', 'Borde', 'Ligamento', 'Hueso'],
    yticklabels=['Fondo', 'Nervio', 'Borde', 'Ligamento', 'Hueso'], 
...
ax2.text(0.5, 0.9, '\n'.join(class_legend), ha='center', va='top', fontsize=12)

# Ajustar diseño antes de guardar
plt.tight_layout()

# Guardar la matriz de confusión como una imagen
plt.savefig(f'/home/tfd/mgutierr/tfm/U-net/Graficas/unet_trans/CM/CM_{n}.png', bbox_inches='tight')
...
plt.savefig(f'/home/tfd/mgutierr/tfm/U-net/Graficas/unet_trans/Graficas/Graph_{n}.png')
...
def mostrar_imagen_y_mascara_con_roi(modelo, imagen, mascara_real, x_roi, indice):
    ...
    if imagen.shape[-1] == 1:  # Escala de grises
        plt.imshow(imagen_muestra.reshape(128, 128), cmap='gray')
    else:  # RGB
    ...
    plt.savefig(f'/home/tfd/mgutierr/tfm/U-net/Graficas/unet_trans/Segmentaciones/{n}_Segmentacion_{indice}.png', bbox_inches='tight')
    plt.close()

mostrar_imagen_y_mascara_con_roi(model, X_validacion, Y_validacion, X_roi_validacion, 0)
mostrar_imagen_y_mascara_con_roi(model, X_validacion, Y_validacion, X_roi_validacion, 1)
mostrar_imagen_y_mascara_con_roi(model, X_validacion, Y_validacion, X_roi_validacion, 2)
mostrar_imagen_y_mascara_con_roi(model, X_validacion, Y_validacion, X_roi_validacion, 3)
mostrar_imagen_y_mascara_con_roi(model, X_validacion, Y_validacion, X_roi_validacion, 4)
mostrar_imagen_y_mascara_con_roi(model, X_validacion, Y_validacion, X_roi_validacion, 5)
mostrar_imagen_y_mascara_con_roi(model, X_validacion, Y_validacion, X_roi_validacion, 6)
mostrar_imagen_y_mascara_con_roi(model, X_validacion, Y_validacion, X_roi_validacion, 7)
mostrar_imagen_y_mascara_con_roi(model, X_validacion, Y_validacion, X_roi_validacion, 8)
mostrar_imagen_y_mascara_con_roi(model, X_validacion, Y_validacion, X_roi_validacion, 9)
