...
model = tf.keras.models.load_model(f'/home/tfd/mgutierr/tfm/U-net/Codigos/unet_trans/modelo_unet_trans_{n}.h5', custom_objects={'weighted_loss': weighted_loss}, safe_mode=False)
...
# def predecir_y_guardar(model, input_dir, output_dir, n_clases=5):
...
#         imagen_input = np.expand_dims(imagen_resized, axis=-1)  # Añadir canal
#         imagen_input = np.expand_dims(imagen_input, axis=0)     # Añadir batch
#         # imagen_input = imagen_input / 255.0                    # Normalizar a [0, 1]
...
#         ax[0].imshow(imagen_resized, cmap='gray')
#         ax[0].set_title("Imagen Original")
#         ax[0].axis('off')
#         ax[1].imshow(prediccion_clase, cmap='viridis')
...
# # Ejecutar la función
# predecir_y_guardar(model, input_dir, output_dir, n_clases=5)